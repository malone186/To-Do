...
### 5. 가입 승인 및 거절 시스템 (Approval/Rejection)
* **가입 승인 대기 (`PENDING`) 흐름**:
  * 신규 회원가입을 마친 유저는 세션 자동 생성이 생략되며 로그인 폼 상단에 가입 신청 완료 메시지가 출력됩니다.
  * 관리자가 승인을 완료하기 전에는 로그인을 시도해도 로그인 처리기 수준에서 검증을 걸어 로그인을 원천 차단합니다.
* **어드민 승인 및 거절 통합 제어**:
...
1. **보안적 우회 차단**:
   ...
   * 가입 신청 대기 상태(`PENDING`) 또는 거절 상태(`REJECTED`)의 계정은 인증 핸들러 수준에서 유효 세션 발급을 사전 차단하여 비승인 유저의 로그인 접촉 시도를 방어합니다.
2. **트랜잭션 안전성**:
