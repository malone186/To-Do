...
export default async function Home({ searchParams }: PageProps) {
  ...
  // 비동기 searchParams 해결 (Next.js 최신 사양 준수)
  ...
  // 1. 유효 To-Do 조회 조건 설정
  const whereCondition: any = session?.role === "ADMIN"
    ? {}
  ...
  // 3. 유니크 카테고리 추출 (사이드바 카테고리 트리 전송용)
  ...
  // 사이드바 진척도용 개수 계산
  const totalCount = rawTodos.length;
  const completedCount = rawTodos.filter((t: any) => t.isCompleted).length;
    ...
    >
      <main className="min-h-screen bg-zinc-50 dark:bg-[#070708] text-zinc-900 dark:text-zinc-100 flex flex-col font-sans relative overflow-hidden pb-16">
        ...
        <div className="container mx-auto px-4 max-w-4xl relative z-10">
          ...
          {rawTodos ? (
            <TodoListContainer initialTodos={rawTodos as any} />
          ) : (
            <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 text-center max-w-md mx-auto">
              ...
              <p className="text-xs text-red-300 mt-1">할 일을 불러오는 도중 오류가 발생했습니다.</p>
            </div>
          )}
        </div>
      </main>
